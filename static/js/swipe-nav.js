document.addEventListener('DOMContentLoaded', () => {
    ...
    // 2. Liquid Glass Setting
    window.applyLiquidGlassSetting = function(enabled) {
        let styleTag = document.getElementById('no-liquid-glass-style');
        if (!enabled) {
            if (!styleTag) {
                styleTag = document.createElement('style');
                styleTag.id = 'no-liquid-glass-style';
                styleTag.innerHTML = `
    ...
    window.applyLiquidGlassSetting(settings.liquid_glass !== false);
    // --- END GLOBAL SETTINGS ---
